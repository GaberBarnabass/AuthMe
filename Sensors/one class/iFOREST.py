...
print(f'contamination:__________{contamination}')
...
oc_forest = IsolationForest(contamination=contamination, random_state=42)
...
for user in uniqueUsers:
    ...
    for trainIndex, testIndex in kf.split(selectedFeatures, yUser):
        ...
        # training phase
        oc_forest.fit(xTrain)
        tp = 0
        tn = 0
        fp = 0
        fn = 0

        # make prediction on normal user
        yPred = oc_forest.predict(xTest)
        yTest = yTest.values
        ...
        # make predictions on anomalies
        yPred = oc_forest.predict(testData)
